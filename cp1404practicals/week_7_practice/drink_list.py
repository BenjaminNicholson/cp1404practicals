from week_7_practice.drink import Drink


class DrinkList:
    def __init__(self):
        self.drinks = []

    def __str__(self):
        return str([str(drink) for drink in self.drinks])

    def __len__(self):
        return len(self.drinks)

    def Add(self, drink):
        self.drinks.append(drink)

    def get_number_alcoholic(self):
        return len([1 for drink in self.drinks if drink.is_alcoholic_drink()])

    def get_alcohol_volume(self):
        return sum([drink.get_alcohol_volume() for drink in self.drinks if drink.is_alcoholic_drink()])

    def get_total_price(self):
        return sum([drink.price for drink in self.drinks])


def run_tests():
    d = DrinkList()
    drink1 = Drink("Pina Colada", 12.30, 12.5, 450)
    drink2 = Drink("Coffee", 4.5, 0, 280)
    d.Add(drink1)
    d.Add(drink1)
    d.Add(drink2)
    print(d)
    print(d.get_number_alcoholic())
    print(d.get_alcohol_volume())


if __name__ == '__main__':
    run_tests()
